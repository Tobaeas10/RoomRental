...
public class Gui extends Application {
	...
	private boolean editBuilding;
	private Building newBuilding;
	private ComboBox<Floor> addBuildingUnusedFloorsComboBox;
	private Label floorsOfBuildingLabel;
	private Button addBuildingAddFloor;
	private Button addBuildingRemoveFloor;
	private ComboBox<Floor> addBuildingUsedFloorsComboBox;
	...
	public void start(Stage window) throws Exception {
		...
		addBuildingButton.setOnAction(e -> onAddBuildingButtonShortCut());
		...
		addBuildingUnusedFloorsComboBox = new ComboBox<Floor>();
		addBuildingUnusedFloorsComboBox.setPromptText("Floors to add");
		addBuildingUnusedFloorsComboBox.setOnAction(e -> onAddBuildingUnusedFloorsComboBox());
		addBuildingGridPane.add(addBuildingUnusedFloorsComboBox, 0, 1, 1, 1);
		
		addBuildingAddFloor = makeButtonWithImage25Height("gui/add.png", "Add Selected Floor to Building");
		addBuildingAddFloor.setOnAction(e -> onAddBuildingAddFloor());
		addBuildingGridPane.add(addBuildingAddFloor, 1, 1, 1, 1);
		
		addBuildingUsedFloorsComboBox = new ComboBox<Floor>();
		addBuildingUsedFloorsComboBox.setPromptText("Floors to remove");
		addBuildingUsedFloorsComboBox.setOnAction(e -> onAddBuildingUsedFloorsComboBox());
		addBuildingGridPane.add(addBuildingUsedFloorsComboBox, 0, 2, 1, 1);
		
		addBuildingRemoveFloor = makeButtonWithImage25Height("gui/minus.png", "Remove Selected Floor to Building");
		addBuildingRemoveFloor.setOnAction(e -> onAddBuildingRemoveFloor());
		addBuildingGridPane.add(addBuildingRemoveFloor, 1, 2, 1, 1);
		
		floorsOfBuildingLabel = new Label();
		floorsOfBuildingLabel.setWrapText(true);
		addBuildingGridPane.add(floorsOfBuildingLabel, 0, 3, 2, 1);
		...
		addBuildingGridPane.add(cancelAddBuildingButton, 0, 4);
		...
		addBuildingGridPane.add(confirmAddBuildingButton, 1, 4);
		...
		for(TableColumn c : roomsTable.getColumns()) {
			if(c.getColumns().size() > 0) {
				for(Object cc : c.getColumns()) {
					((TableColumn) cc).setCellFactory(TooltippedTableCell.forTableColumn());
				}
			}
			c.setCellFactory(TooltippedTableCell.forTableColumn());
		}
	...
	private void onAddBuildingUsedFloorsComboBox() {
		if(addBuildingUsedFloorsComboBox.getValue() != null) addBuildingRemoveFloor.setDisable(false);
		else addBuildingRemoveFloor.setDisable(true);
	}


	private void onAddBuildingUnusedFloorsComboBox() {
		if(addBuildingUnusedFloorsComboBox.getValue() != null) addBuildingAddFloor.setDisable(false);
		else addBuildingAddFloor.setDisable(true);
	}


	private void onAddBuildingRemoveFloor() {
		newBuilding.removeFloor(addBuildingUsedFloorsComboBox.getValue());
		db.unusedFloors.add(addBuildingUsedFloorsComboBox.getValue());
		updateFloorsOfBuildingLabelText();
		addBuildingUnusedFloorsComboBox.getItems().add(addBuildingUsedFloorsComboBox.getValue());
		addBuildingUnusedFloorsComboBox.getSelectionModel().select(addBuildingUsedFloorsComboBox.getValue());
		addBuildingUsedFloorsComboBox.getItems().remove(addBuildingUsedFloorsComboBox.getValue());
		addBuildingUsedFloorsComboBox.getSelectionModel().select(0);
	}


	private void onAddBuildingAddFloor() {
		newBuilding.addFloor(addBuildingUnusedFloorsComboBox.getValue());
		db.unusedFloors.remove(addBuildingUnusedFloorsComboBox.getValue());
		updateFloorsOfBuildingLabelText();
		addBuildingUsedFloorsComboBox.getItems().add(addBuildingUnusedFloorsComboBox.getValue());
		addBuildingUsedFloorsComboBox.getSelectionModel().select(addBuildingUnusedFloorsComboBox.getValue());
		addBuildingUnusedFloorsComboBox.getItems().remove(addBuildingUnusedFloorsComboBox.getValue());
		addBuildingUnusedFloorsComboBox.getSelectionModel().select(0);

	}

	
	private void updateFloorsOfBuildingLabelText() {
		String text = "Floors in " + newBuilding.toString() + ":\n";
		boolean added = false;
		for(Floor f : newBuilding.floors) {
			if(newBuilding.floors.indexOf(f) == newBuilding.floors.size() - 1) {
				//If last index
				text += f.toString();
				added = true;
			} else {
		...
		if(!added) floorsOfBuildingLabel.setText("No floors added yet");
		else floorsOfBuildingLabel.setText(text);
	}


	private void onDeleteBuildingButton() {
		ArrayList<CheckBox> checkBoxes = new ArrayList<CheckBox>();
		checkBoxes.add(new CheckBox("Delete all floors of " + buildingComboBox.getValue().name + "?"));
		checkBoxes.add(new CheckBox("Delete all rooms of " + buildingComboBox.getValue().name + "?"));
		boolean[] answer = PopupWindows.confirmOrCancelWithCheckBoxes("Delete Building Dialogue",
		...
		if(answer[0]) {
			if(!answer[1]) {
				for(Floor f : buildingComboBox.getValue().floors) {
					db.unusedFloors.add(f);
					f.building = null;
					if(answer[2]) f.setRooms(new ArrayList<Room>());
				}
			} else if(!answer[2]) {
				for(Floor f : buildingComboBox.getValue().floors) {
					for(Room r : f.rooms) {
						db.unusedRooms.add(r);
						r.reset();
					}
				}
			}
	...
	private void onConfirmAddBuildingButton() {
		if(!editBuilding) {
			for(Floor f : newBuilding.floors) {
				db.unusedFloors.remove(f);
			}
			db().add(newBuilding);
		}
		
		buildingComboBox.setValue(newBuilding);
		newBuilding = null;		
	...
	private void addBuildingTextFieldTextChanged() {
		...
		newBuilding.name = addBuildingTextField.getText();
	...
	private void onEditBuildingButton() {
		editBuilding = true;
		onAdditBuildingButton();
	}
	private void onAddBuildingButtonShortCut() {
		editBuilding = false;
		onAdditBuildingButton();
	}
	private void onAdditBuildingButton() {
		if(editBuilding) {
			newBuilding = buildingComboBox.getValue();
			addBuildingTextField.setText(buildingComboBox.getValue().name);
		} else {
			newBuilding = new Building("");	
		}
				
		addBuildingUnusedFloorsComboBox.getItems().clear();
		for(Floor f : db.unusedFloors) {
			addBuildingUnusedFloorsComboBox.getItems().add(f);
		}
		addBuildingAddFloor.setDisable(true);
		
		addBuildingUsedFloorsComboBox.getItems().clear();
		for(Floor f : newBuilding.floors) {
			addBuildingUsedFloorsComboBox.getItems().add(f);
		}
		addBuildingRemoveFloor.setDisable(true);
	...
	private ArrayList<Building> getPresetDatabase() {
		...
		db.bookings.add(new Booking(bo1r, new Date(), new Date(), "Holger ist ein männlicher Vorname. Es ist die heutige skandinavische und deutsche Form von Holmger (altnordisch für „Kämpfer von der Insel“ beziehungsweise „treuer Speerkämpfer“; zusammengesetzt aus holmi [„Insel“] und geirr [„Speer“])."));

		db.unusedFloors.add(new Floor(null, "Ein wilder floar"));
		db.unusedFloors.add(new Floor(null, "Ein gaaanz wilder floar"));
		db.unusedFloors.add(new Floor(null, "Iuaoo Beep Boop"));
		
		db.unusedRooms.add(new Room(null, "Ich bin ein Raum :D"));
		db.unusedRooms.add(new Room(null, "Ich auch :DDD"));
		db.unusedRooms.add(new Room(null, "(a+b)² = a²+2ab+b²"));
		db.unusedRooms.add(new Room(null, "(a-b)² = a²-2ab+b²"));
		db.unusedRooms.add(new Room(null, "(a+b) * (a-b) = a²+b²"));
		db.unusedRooms.add(new Room(null, "P(A|B) = P(A&B) / P(B)"));
